...
if nargout == 0
    ...
    T2 = 8:10;
    figure;
    subplot(5,2,1);imagesc(GxT_PL);ylabel('seg length');xlabel('trials');
    subplot(5,2,2);plot(-12:1:12,mean(GxT_PL(:,T1),2,'omitnan'),'b',-12:1:12,mean(GxT_PL(:,T2),2,'omitnan'),'r');ylabel('path length (cm)');xlabel('seg length');
    subplot(5,2,3);imagesc(GxT_N);ylabel('seg length');xlabel('trials');
    subplot(5,2,4);plot(-12:1:12,mean(GxT_N(:,T1),2),'b',-12:1:12,mean(GxT_N(:,T2),2),'r');ylabel('# of segments per trial');xlabel('seg length');
    subplot(5,2,5);imagesc(VxT_N);ylabel('door#');xlabel('trials');
    subplot(5,2,6);plot(-11:1:12,mean(VxT_N(:,T1),2)/sum(mean(VxT_N(:,T1),2))*100,'b',-11:1:12,mean(VxT_N(:,T2),2)/sum(mean(VxT_N(:,T2),2))*100,'r');ylabel('% of visits');xlabel('door#');
    subplot(5,2,7);imagesc(NxT_serial);xlabel('trials');ylabel('serial bout length');
    subplot(5,2,8);plot(1:20,mean(NxT_serial(:,T1),2),'b',1:20,mean(NxT_serial(:,T2),2),'r');xlabel('serial bout length');ylabel('% of serial bouts');
    subplot(5,2,9);imagesc(NsegDistxT);xlabel('trials');ylabel('trial length');
    subplot(5,2,10);plot(1:50,mean(NsegDistxT(:,T1),2),'b',1:50,mean(NsegDistxT(:,T2),2),'r');xlabel('trial length (seg#)');ylabel('% of trials');
end
